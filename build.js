 ...
 *
 * Output: dist/MCA_Hunt_Survey.html — one file with all CSS, JS, the reference
 * lists and the icon inlined, so it runs when opened straight from disk on a
 * phone (no web server, no separate files, works fully offline).
 *
 * Usage:  node build.js
 * Re-run this whenever you edit the source in js/, css/ or data/reference.json
 * (for example after dropping in the real Zone/Ward lists).
 */
const fs = require('fs');
...
<head>
  ...
  <meta name="theme-color" content="#256d40">
  <title>MCA Hunting Survey</title>
...
const outFile = path.join(outDir, 'MCA_Hunt_Survey.html');
